...
const Dashboard = () => {
    ...
    // Logout function to clear localStorage and navigate to sign-in
    const logout = () => {
        localStorage.removeItem('user:detail'); // Clear user data from localStorage
        localStorage.removeItem('user:token'); // Clear token from localStorage
        navigate('/users/sign_in'); // Redirect to the sign-in page
    }

	useEffect(() => {
		// Establish Socket.IO connection after component mount
		...
		<div className='w-screen flex'>
			<div className='w-[25%] h-screen bg-secondary overflow-scroll'>
				...
				<div className='mx-14 mt-10'>
					...
					<button className='bg-blue-500 text-white px-4 py-2 rounded mt-4' onClick={logout}>Logout</button>
					<div>
					{
					...
					<div className='p-14'>
						{
							messages?.messages?.length > 0 ?
								messages.messages.map(({ message, user: { id } = {} },index) => {
									return (
										<>
										<div  key={id || index} className={`max-w-[40%] rounded-b-xl p-4 mb-6 ${id === user?.id ? 'bg-primary text-white rounded-tl-xl ml-auto' : 'bg-secondary rounded-tr-xl'} `}>{message}</div>
